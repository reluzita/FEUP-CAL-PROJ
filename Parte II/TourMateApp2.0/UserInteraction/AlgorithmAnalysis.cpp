...
void compareDFSBFS(Graph<coord> &g){
    ...
    cout << "BFS found " << bfsPoints.size() << " nodes from source " << idStrt << " in " << t1.count()- t0.count()<< " milliseconds"<< endl;
    cout << "DFS found "<< dfsPoints.size() << " nodes from source " << idStrt << " in " << t2.count()- t1.count() << " milliseconds"<< endl;
